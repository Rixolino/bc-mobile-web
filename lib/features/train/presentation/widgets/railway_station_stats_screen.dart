...
class _TrainStatsScreenState extends State<TrainStatsScreen> {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          _isLoading
              ? Padding(padding: const EdgeInsets.symmetric(horizontal: 16.0), child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Theme.of(context).textTheme.bodyLarge?.color)))
              : IconButton(icon: Icon(Icons.refresh, color: Theme.of(context).textTheme.bodyLarge?.color), onPressed: _refreshStats),
        ],
      ),
            ...
            Center(
              child: Column(
                children: [
                  ...
                  Text("${stationRating.toStringAsFixed(1)} / 5.0", style: TextStyle(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.7))),
                ],
              ),
            ),
  ...
  Widget _buildSectionTitle(String title) => Text(title, style: TextStyle(color: Theme.of(context).textTheme.bodyLarge?.color, fontSize: 18, fontWeight: FontWeight.bold));

  Widget _buildEmptyNotice() => Text(RuntimeLocalizations.t(context, 'stats_no_data'), style: TextStyle(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.4), fontSize: 14));
  ...
  Widget _buildKPI(String title, String val, Color color) {
    return Container(
      ...
      child: Column(children: [Text(val, style: TextStyle(color: Theme.of(context).textTheme.bodyLarge?.color, fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 4), Text(title, style: TextStyle(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.6), fontSize: 13), textAlign: TextAlign.center)]),
    );
  }
  ...
  Widget _buildSubCard({required String title, required Color color, required List<Widget> children}) {
    return Container(
      ...
      decoration: BoxDecoration(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.04), borderRadius: BorderRadius.circular(16), border: Border.all(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.08))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Container(width: 8, height: 8, decoration: BoxDecoration(color: color, shape: BoxShape.circle)), const SizedBox(width: 8), Text(title, style: TextStyle(color: Theme.of(context).textTheme.bodyLarge?.color, fontWeight: FontWeight.bold, fontSize: 14))]), Divider(color: Theme.of(context).dividerColor, height: 16), ...children]),
    );
  }

  Widget _buildMiniRow(String label, String value) {
    return Padding(padding: const EdgeInsets.symmetric(vertical: 2.0), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(label, style: TextStyle(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.5), fontSize: 12)), Text(value, style: TextStyle(color: Theme.of(context).textTheme.bodyLarge?.color, fontWeight: FontWeight.w600, fontSize: 12))]));
  }
        ...
        itemBuilder: (context, index) {
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.04),
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.08)),
            ),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text("$hour:00", style: TextStyle(color: Theme.of(context).textTheme.bodyLarge?.color, fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 12),
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                ...
                const SizedBox(height: 12),
                Text("Rischio Ritardo", style: TextStyle(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.5), fontSize: 11)),
...
Widget _buildChart(List<dynamic> data) {
      ...
      child: LineChart(
        LineChartData(
              ...
              getTooltipItems: (List<LineBarSpot> touchedSpots) {
                return touchedSpots.map((barSpot) {
                  ...
                  return LineTooltipItem(
                    "$titleLabel\n",
                    TextStyle(color: Theme.of(context).textTheme.bodyMedium?.color?.withOpacity(0.7), fontSize: 11, fontWeight: FontWeight.normal),
                    children: [
                      TextSpan(
            ...
            leftTitles: AxisTitles(
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) => Padding(
                  ...
                  child: Text("${value.toInt()}m", style: TextStyle(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.5), fontSize: 11, fontWeight: FontWeight.bold), textAlign: TextAlign.right)
                )
              )
            ),
          ...
          gridData: FlGridData(
            ...
            getDrawingHorizontalLine: (value) => FlLine(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.06), strokeWidth: 1), 
            getDrawingVerticalLine: (value) => FlLine(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.06), strokeWidth: 1)
          ),
          borderData: FlBorderData(
            show: true, 
            border: Border(bottom: BorderSide(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.2), width: 1.5), left: BorderSide(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.2), width: 1.5), right: BorderSide.none, top: BorderSide.none)
          ),
        ),
      ),
    );
  ...
  Widget _buildCategoryTile(dynamic cat) {
    ...
    return Container(
      ...
      child: Column(children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("${cat['category']} (${cat['totalTrains']} treni)", style: TextStyle(color: Theme.of(context).textTheme.bodyLarge?.color, fontWeight: FontWeight.bold)), Text(RuntimeLocalizations.t(context, 'stats_avg_delay_val', params: {'val': cat['averageDelay'].toString()}), style: TextStyle(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.7), fontSize: 13))]), const SizedBox(height: 8), Row(children: [Expanded(child: LinearProgressIndicator(value: onTime / 100, backgroundColor: Theme.of(context).dividerColor, color: progressColor)), const SizedBox(width: 12), Text(RuntimeLocalizations.t(context, 'stats_punctuality_val', params: {'val': onTime.toString()}), style: TextStyle(color: progressColor, fontSize: 12, fontWeight: FontWeight.bold))])]),
    );
  }
  ...
  Widget _buildWorstTrainTile(dynamic train) {
    return Container(
      ...
      child: ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4), title: Text(train['line'] ?? RuntimeLocalizations.t(context, 'stats_unknown_train'), style: TextStyle(color: Theme.of(context).textTheme.bodyLarge?.color, fontWeight: FontWeight.bold)), subtitle: Text(RuntimeLocalizations.t(context, 'stats_detected_times', params: {'count': train['samples'].toString()}), style: TextStyle(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.5), fontSize: 12)), trailing: Text("+${train['averageDelay']} min", style: const TextStyle(color: Colors.redAccent, fontSize: 16, fontWeight: FontWeight.bold))),
    );
  }
  ...
  Widget _buildWorstRouteTile(dynamic route) {
    return Container(
      ...
      child: ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4), title: Text(route['destination'] ?? RuntimeLocalizations.t(context, 'stats_destination'), style: TextStyle(color: Theme.of(context).textTheme.bodyLarge?.color, fontWeight: FontWeight.w600, fontSize: 14)), subtitle: Text(RuntimeLocalizations.t(context, 'stats_analyzed_trips', params: {'count': route['totalTrips'].toString()}), style: TextStyle(color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.5), fontSize: 12)), trailing: Text("+${route['averageDelay']} min", style: const TextStyle(color: Colors.orange, fontSize: 15, fontWeight: FontWeight.bold))),
    );
  }
}